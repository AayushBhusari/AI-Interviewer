...
app.get("/api/user/profile", verifyAuthToken, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      "SELECT id, name, email, surname, age, course, qualifications, goals, created_at FROM users WHERE id = $1",
      [userId],
    );
...
app.post("/api/user/profile", verifyAuthToken, async (req, res) => {
  try {
    const userId = req.userId;
    const { name, surname, age, course, qualifications, goals } = req.body;
    ...
    const result = await pool.query(
      ...
      [name, surname, age, course, qualifications, goals, userId],
    );
...
`;
    ...
    try {
      ...
      feedbackReport = JSON.parse(jsonMatch ? jsonMatch[0] : feedbackText);
    } catch (parseErr) {
